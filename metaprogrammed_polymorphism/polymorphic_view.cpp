...
namespace polymorphic {
	namespace detail {
		...
		constexpr auto get_vtable_entry(TypedSignature) {
			return [](TypedSignature, const vtable_entry* table) {
				return	table[Index];
			};
		}
		...
		constexpr auto make_vtable_callers(std::index_sequence<I...>, TypedSignatures...) {
			return  overload{ get_vtable_caller<I>(TypedSignatures{})... };
		}

		template<size_t... I, typename... TypedSignatures>
		constexpr auto make_vtable_getter(std::index_sequence<I...>, TypedSignatures...) {
			return  overload{ get_vtable_entry<I>(TypedSignatures{})... };
		}


		template<typename... Signatures>
		constexpr auto vtable_callers = make_vtable_callers(std::make_index_sequence<sizeof...(Signatures)>{}, type<Signatures>{}...);

		template<typename... Signatures>
		constexpr auto vtable_getters = make_vtable_getter(std::make_index_sequence<sizeof...(Signatures)>{}, type < Signatures>{}...);
		...
		struct polymorphic_caller_implementation {
			...
			decltype(auto) call_vtable(Method method, Ptr p, Parameters&&... parameters)const {
				return vtable_callers<Signatures...>(method, vptr_, p,
					std::forward<Parameters>(parameters)...);
			}

			template <typename... OtherSignatures>
			static const vtable_entry* get_vtable(const polymorphic_caller_implementation<OtherSignatures...>& other) {
			...
			template <typename... OtherSignatures>
			polymorphic_caller_implementation(const polymorphic_caller_implementation<OtherSignatures...>& other) :vptr_(get_vtable(other)) {}
		};
