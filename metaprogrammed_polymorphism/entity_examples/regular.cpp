...
template <typename T>
struct entity_imp : entity_interface {
...
int main() {
  ...
  entities.emplace_back(5);
  entities.emplace_back(std::string("Hello"));
  entities.emplace_back(my_clock());
  ...
  entities[1] = std::string("World");

  for (auto& e : entities) {
    e.draw(std::cout);
  }
  for (auto& e : entities) {
    e.debug_draw(std::cout);
  }
